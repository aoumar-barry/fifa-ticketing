...
describe("Admin CRUD Matches integration tests", () => {
  ...
  beforeAll(async () => {
    ...
    await mongoose.connect(mongod.getUri(), {
      dbName: "fifa-ticketing-admin-test",
    });
  ...
  describe("Role-based access protection", () => {
    it("should return 401 Unauthorized if no token is provided", async () => {
      await request(app).get("/api/v1/admin/matches").expect(401);

      await request(app).post("/api/v1/admin/matches").send({}).expect(401);
    });
  ...
  describe("Sales Stats and CSV Export operations", () => {
    ...
    it("should calculate sales stats correctly for admin", async () => {
      ...
      const statsMatch1 = res.body.matchStats.find(
        (s) => s.match.id === match1._id.toString(),
      );
    ...
    it("should generate CSV export correctly for admin", async () => {
      ...
      expect(csvLines[0]).toBe(
        "Order ID,Ticket ID,Buyer Email,Match,Date,Stadium,Section,Row,Seat Number,Category,Price",
      );
      ...
      expect(res.text).toContain("USA vs Germany");
      expect(res.text).toContain("France vs Japan");
